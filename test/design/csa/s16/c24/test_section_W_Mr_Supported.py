...
def test_W_Major():
    ...
    section = ls.getByName(steelSections, 'W310x74')
    Mr = s16.getSectionMrSupported(section) / 1000
    MrSol = 366
    assert Mr == pytest.approx(MrSol, rel = 0.01)
           
    section = ls.getByName(steelSections, 'W1100x390')
    Mr = s16.getSectionMrSupported(section) / 1000
    MrSol = 6460
    assert Mr == pytest.approx(MrSol, rel = 0.01)
        
    section = ls.getByName(steelSections, 'W530x72')
    Mr = s16.getSectionMrSupported(section) / 1000
    MrSol = 472
    assert Mr == pytest.approx(MrSol, rel = 0.01)
